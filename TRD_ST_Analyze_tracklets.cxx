...
//----------------------------------------------------------------------------------------
Int_t Ali_TRD_ST_Analyze::Do_TPC_TRD_matching_allEvents(Double_t xy_matching_window, Double_t z_matching_window)
{
...
//----------------------------------------------------------------------------------------
Int_t Ali_TRD_ST_Analyze::Do_TRD_self_matching(Long64_t i_event, double offset_window, double angle_window)
{
    ...
    for (int i_layer=5; i_layer>3; i_layer--)
    {
        ...
        for (int a_det=(539-(5-i_layer)); a_det>=0; a_det-=6)
        {
            ...
            for (int a_trkl=0; a_trkl<(Int_t)tracklet_dir[a_det].size(); a_trkl++)
            {
                ...
                for (int b_trkl=0; b_trkl<(Int_t)tracklet_offset[b_det].size(); b_trkl++)
                {
                    ...
                    for (int c_det=b_det-1; c_det>(a_det-6); c_det--)
                    {
                        ...
                        for (int c_trkl=0; c_trkl<(Int_t)tracklet_offset[c_det].size(); c_trkl++)
                        {
                        ...
                        int count = 0;
                        for (int trkl=0; trkl<(Int_t)offset_diffs.size(); trkl++)
                        {
                    ...
                    for (int t=0; t<(Int_t)tracklet_chain.size(); t++)
                    {
                    ...
                    if ((Int_t)tracklet_chain.size()+2 >= 5 && offset_diff_sum/(Int_t)tracklet_chain.size() < 10 && angle_diff_sum/(Int_t)tracklet_chain.size() < 30)
                    {
                        ...
                        for (int t=2; t<(Int_t)tracklet_chain.size(); t++)
                        {
    ...
    for (int i=0; i<(Int_t)tracks.size(); i++)
    {
        cout << endl << "track: " << i << endl;
        for (int j=0; j<(Int_t)tracks[i].size(); j++)
        {
    ...
    for (int i_track=0; i_track<(Int_t)tracks.size(); i_track++)
    { 
        for (int i_tracklet=0; i_tracklet<(Int_t)tracks[i_track].size(); i_tracklet++)
        {
            ...
            vec_TEveLine_self_matched_tracklets[i_track].resize((Int_t)tracks[i_track].size());
