...
Ali_TRD_ST_Analyze::Ali_TRD_ST_Analyze(TString out_dir, TString out_file_name, Int_t graphics)
{
    ...
    hist_phi = new TH1D("hist_phi", "hist_phi", 720, -360, 360);
    hist_dcax_dcay = new TH2D("hist_dcax_dcay", "hist_dcax_dcay", 720, -360, 360, 100, -50, 50);
    hist_pt_dca_phi = new TH2D("hist_pt_dca_phi", "hist_pt_dca_phi", 100, 0, 2, 200, -3, 3);
    hist_pt_dca_x = new TH2D("hist_pt_dca_x", "hist_pt_dca_x", 100, 0, 2, 200, -50, 50);
    hist_pt_dca_y = new TH2D("hist_pt_dca_y", "hist_pt_dca_y", 100, 0, 2, 200, -50, 50);
    hist_pt_dca_xy = new TH2D("hist_pt_dca_xy", "hist_pt_dca_xy", 100, 0, 2, 200, -50, 50);
    hist_pt_dca_z = new TH2D("hist_pt_dca_z", "hist_pt_dca_z", 100, 0, 2, 200, -50, 50);
    hist_pt_dca = new TH2D("hist_pt_dca", "hist_pt_dca", 100, 0, 2, 100, 0, 50);
    hist_dEdx_dca = new TH2D("hist_dEdx_dca", "hist_dEdx_dca", 1000, 0, 300, 200, 0, 50);
    
    eventno = 0;
    
    matchfile = TFile::Open("./ST_out/matchprops.root", "RECREATE");
    matchtracks = new TTree("T", "matched_tracks_properties");
    matchtracks->Branch("TPC_track_id", &tpcid);
    matchtracks->Branch("track_pos_at_tracklet", &tpat);
    matchtracks->Branch("vec_TV3_offset_tracklets", &vTV3ot);
    matchtracks->Branch("detector", &dettree);
    matchtracks->Branch("charge", &chargetree);
    matchtracks->Branch("pT_track", &pttree);
    matchtracks->Branch("dca", &dcatree);
    matchtracks->Branch("NITScls", &NITSclstree);
    matchtracks->Branch("NTPCcls", &NTPCclstree);
    matchtracks->Branch("NTRDcls", &NTRDclstree);
    matchtracks->Branch("chi2", &chi2tree);
    matchtracks->Branch("dEdx", &dEdxtree);
    //some additional branches
    matchtracks->Branch("nsigma_TPC_e", &nsigma_TPC_e_tree);
    matchtracks->Branch("nsigma_TPC_pi", &nsigma_TPC_pi_tree);
    matchtracks->Branch("nsigma_TPC_p", &nsigma_TPC_p_tree);
    matchtracks->Branch("nsigma_TPC_K", &nsigma_TPC_K_tree);
    //matchtracks->Branch("nsigma_TOF_e", &nsigma_TOF_e_tree);
    //matchtracks->Branch("nsigma_TOF_pi", &nsigma_TOF_pi_tree);
    matchtracks->Branch("TRD_signal", &TRD_signal_tree);
    matchtracks->Branch("TRDsumADC", &TRDsumADC_tree);
    matchtracks->Branch("TLV_part", &TLV_part_tree);
    matchtracks->Branch("TOFsignal", &TOFsignal_tree);
    matchtracks->Branch("Track_length", &Track_length_tree);
    matchtracks->Branch("momentum", &momentum_tree);
    matchtracks->Branch("eta_track", &eta_track_tree);
    matchtracks->Branch("theta_track", &theta_track_tree);
    matchtracks->Branch("phi_track", &phi_track_tree);
        ...
        if(in)
        {
            ...
            while(in)
            {
                ...
                if(str[0] != 0)
                {
                    ...
                    cout << TRD_ST_TREE.Data() << endl;
                    Long64_t file_entries = input_SE->GetEntries();
                    cout << "File added to data chain: " << addfile.Data() << " with " << (file_entries-entries_save) << " entries" << endl;
                    entries_save = file_entries;
                }
            }
            Int_t setbranchvalue = input_SE  ->SetBranchAddress( TRD_ST_BRANCH, &TRD_ST_Event );
            cout << setbranchvalue << endl;
        }
        else
        {
...
Int_t Ali_TRD_ST_Analyze::Do_TPC_TRD_matching(Long64_t i_event, Double_t xy_matching_window, Double_t z_matching_window, Int_t graphics)
{
        ...
        //for(Int_t i_track = 3; i_track < 4; i_track++)
    {
        ...
        for(Int_t i_layer = 0; i_layer < 6; i_layer++)
        {
            ...
            Double_t dist_min      = 50.0;
            Double_t dist_min_perp = 50.0;
            Double_t dist          = 0.0;
            Double_t dist_perp	   = 0.0;
            Int_t    det_best      = -1;
			...
			//if(dist_min_perp < 4.) continue;
			//if(dist_min<6.)continue;
...
//printf("tracklet TRD index: %d, i_track: %d, i_layer: %d \n",TRD_index_tracklets[det_best][tracklet_best],tracks_size,matched_tracks[tracks_size].size() - 1);
            ...
            Double_t phi_TPC = TMath::ATan2(track_pos_at_tracklet[1], track_pos_at_tracklet[0])*(180.0/3.141592653589793238463);
            Double_t phi_TRD = TMath::ATan2(vec_TV3_offset_tracklets[det_best][tracklet_best][1], vec_TV3_offset_tracklets[det_best][tracklet_best][0])*(180.0/3.141592653589793238463);
            Double_t TPC_TRD_angle = phi_TPC-phi_TRD;
            TPC_TRD_angle = std::fmod(TPC_TRD_angle, 180) - TMath::Sign(1, TPC_TRD_angle)*(TPC_TRD_angle >= 180)*180;
            Double_t TPC_TRD_angle_sign = TMath::Sign(1, TPC_TRD_angle);

			Double_t res_xy = 0;
			Double_t res = 0;
			for(Int_t i_xyz = 0; i_xyz < 3; i_xyz++){
				res += residuals[i_xyz]*residuals[i_xyz];
				if(i_xyz<2)res_xy += residuals[i_xyz]*residuals[i_xyz];
			}
			res_xy = TMath::Sqrt(res_xy)*TPC_TRD_angle_sign;
			res = TMath::Sqrt(res);

			vector<Int_t> trackid{eventno, i_track};
			tpcid = trackid;
			tpat = track_pos_at_tracklet;
			vTV3ot = vec_TV3_offset_tracklets[det_best][tracklet_best];
			dettree = det_best;
			chargetree = charge;
			pttree = pT_track;
			dcatree = dca;
			...
			NTRDclstree = NTRDcls;
			dEdxtree = TPCdEdx;
			nsigma_TPC_e_tree = nsigma_TPC_e;
			nsigma_TPC_pi_tree = nsigma_TPC_pi;
			nsigma_TPC_p_tree = nsigma_TPC_p;
			nsigma_TPC_K_tree = nsigma_TPC_K;
			//nsigma_TOF_e_tree = nsigma_TOF_e;
			//nsigma_TOF_pi_tree = nsigma_TOF_pi;
			TRD_signal_tree = TRD_signal;
			TRDsumADC_tree = TRDsumADC;
			TLV_part_tree = TLV_part;
			TOFsignal_tree = TOFsignal;
			Track_length_tree = Track_length;
			momentum_tree = momentum;
			eta_track_tree = eta_track;
			theta_track_tree = theta_track;
			phi_track_tree = phi_track;
			matchtracks->Fill();

			hist_phi->Fill(phi_TPC);
			hist_dcax_dcay->Fill(phi_TPC, residuals[0]);
			hist_pt_dca_phi->Fill(pT_track, TPC_TRD_angle);
			hist_pt_dca_x->Fill(pT_track, residuals[0]);
			hist_pt_dca_y->Fill(pT_track, residuals[1]);
			hist_pt_dca_xy->Fill(pT_track, res_xy);
			hist_pt_dca_z->Fill(pT_track, residuals[2]);
            hist_pt_dca->Fill(pT_track, res);
            hist_dEdx_dca->Fill(TPCdEdx, res);
            //------------------------------------------
...
void Ali_TRD_ST_Analyze::Write()
{
    ...
    hist_phi->Write();
    hist_dcax_dcay->Write();
    hist_pt_dca_phi->Write();
    hist_pt_dca_x->Write();
    hist_pt_dca_y->Write();
    hist_pt_dca_xy->Write();
    hist_pt_dca_z->Write();
    hist_pt_dca->Write();
    hist_dEdx_dca->Write();
    for(Int_t i_det = 0; i_det < 540; i_det++)
